...
comparison_case <- outlier_cases_1000 %>%
  mutate(compare_case = if_else(cases_per_1000 >= mean_case_per_1000, 1, 0))%>%
  dplyr::select(county, compare_case)
# Display the new dataframe
print(comparison_case)

# Ensure dplyr is loaded and then run the code with dplyr::select
comparison_death <- outlier_cases_1000 %>%
  mutate(compare_death = if_else(deaths_per_1000 >= mean(deaths_per_1000, na.rm = TRUE), 1, 0)) %>%
  dplyr::select(county, compare_death)

# Display the resulting dataframe
print(comparison_death)

# Add an index column to each dataframe
cluster_group_1_index <- cluster_group_1 %>%
  mutate(index = row_number())

comparison_case_index <- comparison_case %>%
  mutate(index = row_number())

# Join the dataframes by the index column
joined_df <- cluster_group_1_index %>%
  left_join(comparison_case_index, by = "index")

# Display the resulting dataframe
print(joined_df)

# Add the cluster assignments as a new column to the joined_df
joined_df <- joined_df %>%
  mutate(cluster = factor(km_model_1$cluster))

# Display the updated joined_df with the cluster assignments
print(joined_df)
              
# Step 1: Create a contingency table for `compare_case` and `cluster`
contingency_table <- table(joined_df$compare_case, joined_df$cluster)

# Step 2: Calculate purity
# For each cluster, take the maximum count in the table, then sum these max values
max_counts <- apply(contingency_table, 2, max)
purity <- sum(max_counts) / nrow(joined_df)

# Print the purity score
print(paste("Purity:", round(purity, 4)))


# Add an index column to each dataframe
cluster_group_2_index <- cluster_group_2 %>%
  mutate(index = row_number())
...
# Join the dataframes by the index column
joined_df_2 <- cluster_group_2_index %>%
  left_join(comparison_case_index, by = "index")

# Display the resulting dataframe
print(joined_df_2)

# Apply k-means clustering for cluster_group_2 with 4 clusters (example)
km_model_2 <- kmeans(cluster_group_2, centers = 4, nstart = 10)

# Add the cluster assignments as a new column to the joined_df_2
joined_df_2 <- joined_df_2 %>%
  mutate(cluster = factor(km_model_2$cluster))

# Display the updated joined_df_2 with the cluster assignments
print(joined_df_2)

# Step 1: Create a contingency table for `compare_case` and `cluster` for cluster_group_2
contingency_table_2 <- table(joined_df_2$compare_case, joined_df_2$cluster)

# Step 2: Calculate purity for cluster_group_2
# For each cluster, take the maximum count in the table, then sum these max values
max_counts_2 <- apply(contingency_table_2, 2, max)
purity_2 <- sum(max_counts_2) / nrow(joined_df_2)

# Print the purity score for cluster_group_2
print(paste("Purity for cluster_group_2:", round(purity_2, 4)))
